...
- SSH 连接时实时显示远程输出，60 秒无响应会给出带日志的超时提示

## 服务器预填（不提交到 GitHub）

首次进入后，「添加远程服务器」只需填名称 / 主机 / 用户名，其余有默认值。
连接管理页支持 **「导入服务器配置…」**：把 `server-presets.json`（JSON 数组，字段同连接表单）传到手机，
在 App 里选该文件即可批量导入服务器，不用手输。

服务器预填文件只存在手机上（应用私有目录或用户自己导入的文件），**不会**进入 GitHub 仓库；
仓库里只有一个不含真实信息的占位模板 `app/src/main/assets/server-presets.json`。
